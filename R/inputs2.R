...
## throughout this class, not just within the 'data' element.

## TODO: we'll need the ability to reroute inputs too, so that the
## query parameter MyThing becomes the R argument my_thing for
## example, but more generally.  This requires extending the interface
## just a little to allow two names that have one as the other's
## default; this is only an option though for non-body parameters!
